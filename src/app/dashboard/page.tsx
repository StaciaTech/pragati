  ...
  if (role !== ROLES.INNOVATOR) {
    ...
    return (
      <Card>
        ...
        <CardContent>
          <p>
            Your role-specific dashboard is available via the sidebar
            navigation.
          </p>
        </CardContent>
      </Card>
      ...
      <Card className="w-full bg-gradient-to-br from-primary to-primary/80 text-primary-foreground shadow-lg border-0">
        <CardHeader>
          ...
          <CardDescription className="text-primary-foreground/80">
            Ready to change the world? Let's get your next great idea validated.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button
          ...
          {ideas.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-muted-foreground">
                You haven't submitted any ideas yet. Go to "Submit Idea" to get
                started!
              </p>
            </div>
          ) : (
                      ...
                      <TableCell className="text-right">
                        {idea.report ? (
                          <Button variant="link" asChild className="p-0 h-auto">
                            <Link
                              href={`/dashboard/ideas/${idea.id}?role=${ROLES.INNOVATOR}`}
                            >
                              View Report
                            </Link>
                          </Button>
