...
export default function PrincipalDashboardPage() {
  ...
  const topInnovators = MOCK_INNOVATORS.filter(
    (innovator) => innovator.collegeId === college.id
  )
    .map((innovator) => {
      const innovatorIdeas = MOCK_IDEAS.filter(
        (i) => i.innovatorEmail === innovator.email && i.report?.overallScore
      );
      const ideasCount = innovatorIdeas.length;
      if (ideasCount === 0) return null;
      const avgScore =
        innovatorIdeas.reduce((sum, i) => sum + i.report!.overallScore, 0) /
        ideasCount;
      return {
        name: innovator.name,
        score: avgScore,
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-center">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xl font-bold">
              {totalCredits - college.creditsAvailable} /{" "}
                  ...
                  <Pie
                    ...
                    label={({
                      ...
                      return (
                        ...
                        >{`${statusData[index].name} (${value})`}</text>
                      );
                    }}
                  >
