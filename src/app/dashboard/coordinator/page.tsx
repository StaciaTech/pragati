...
export default function CoordinatorDashboardPage() {
  ...
  const [topInnovators, setTopInnovators] = useState([]);
  const apiUrl = process.env.NEXT_PUBLIC_API_URL;
  const token = localStorage.getItem("token");

  const fetchTopPerformingInno = async () => {
    try {
      const { data } = await axios.get(
        `${apiUrl}/api/analytics/top-innovators`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      setTopInnovators(data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  useEffect(() => {
    fetchTopPerformingInno();
  }, []);

  const assignedIdeas = MOCK_IDEAS.filter(
    (idea) => idea.ttcAssigned === userTTC.id
  );
  const pendingEvaluations = assignedIdeas.filter(
    (idea) => idea.status === "Pending" || !idea.report
  );
  const scheduledConsultations = assignedIdeas.filter(
    (idea) => idea.consultationStatus === "Scheduled"
  );
  ...
  //     .map((innovator) => {
  //       const innovatorIdeas = assignedIdeas.filter(
  //         (idea) => idea.innovatorEmail === innovator.email && idea.report
  //       );
  //       if (innovatorIdeas.length === 0) return null;
  //       const avgScore =
  //         innovatorIdeas.reduce(
  //           (sum, idea) => sum + (idea.report?.overallScore || 0),
  //           0
  //         ) / innovatorIdeas.length;
  //       return {
  //         name: innovator.name,
  //         score: avgScore,
  //       };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Link
          href={`/dashboard/coordinator/innovator-management?role=TTC Coordinator`}
        >
        ...
        <Link
          href={`/dashboard/coordinator/consultations?role=TTC Coordinator`}
        >
          <Card className="hover:shadow-lg transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <Card
          ...
          onClick={() =>
            router.push(`/dashboard/coordinator/analytics?role=TTC Coordinator`)
          }
        >
          <CardHeader>
            <CardTitle>Top Performing Innovators</CardTitle>
            <CardDescription>
              Innovators you manage, ranked by average idea score.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        >
          <CardHeader>
            ...
            <CardDescription>
              Breakdown of outcomes for ideas you've been assigned.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Your scheduled consultations with innovators.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {scheduledConsultations.length > 0 ? (
                scheduledConsultations.map((idea) => (
                  <TableRow
                    ...
                    onClick={() =>
                      router.push(
                        `/dashboard/ideas/${idea.id}?role=TTC Coordinator`
                      )
                    }
                  >
